...
String buffering()
{
  ...
  for (int i = 1; i < sizeofdataholder; i++)
  {
    ...
    {
      str += data_holder_coordinates[i][0];
      str += " ";
      str += data_holder_coordinates[i][1];
      str += "\r\n";
    }
  }
  return str;
...
void insertToDataHolder(float angle_s, float angle_e, int values[12], int strenght[12])
{
  ...
  if (angle_s > angle_e)
  {
    step = (angle_e - 360 + angle_s) / N_POINTS;
  }
  else
  {
    step = (angle_e - angle_s) / N_POINTS;
  }
  for (int i = 0; i < N_POINTS; i++)
  {
    if (strenght[i] > 100 && values[i] > 0)
    { // dato accettabile
      if (angle_s + step * i > 360)
      {
        data_holder_coordinates[data_holder_counter][0] = (angle_s - 360 + step * i) * 100;
      } // Se il dato supera i 360 gradi tra inizio e fine
      else
      {
        data_holder_coordinates[data_holder_counter][0] = (angle_s + step * i) * 100;
      } // Se non supera i 360°
      data_holder_coordinates[data_holder_counter][1] = values[i];
      data_holder_counter += 1;
      if (data_holder_counter >= sizeofdataholder)
      {
